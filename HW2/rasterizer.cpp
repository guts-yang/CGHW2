// clang-format off
//
// Created by goksu on 4/6/19.
//

#include <algorithm>
#include <vector>
#include "rasterizer.hpp"
#include <opencv2/opencv.hpp>
#include <math.h>


rst::pos_buf_id rst::rasterizer::load_positions(const std::vector<Eigen::Vector3f> &positions)
{
    auto id = get_next_id();
    pos_buf.emplace(id, positions);

    return {id};
}

rst::ind_buf_id rst::rasterizer::load_indices(const std::vector<Eigen::Vector3i> &indices)
{
    auto id = get_next_id();
    ind_buf.emplace(id, indices);

    return {id};
}

rst::col_buf_id rst::rasterizer::load_colors(const std::vector<Eigen::Vector3f> &cols)
{
    auto id = get_next_id();
    col_buf.emplace(id, cols);

    return {id};
}

auto to_vec4(const Eigen::Vector3f& v3, float w = 1.0f)
{
    return Vector4f(v3.x(), v3.y(), v3.z(), w);
}


static bool insideTriangle(int x, int y, const Vector3f* _v)
{
    // ����ת��Ϊ������
    float xf = x + 0.5f;
    float yf = y + 0.5f;

    Eigen::Vector2f p(xf, yf);

    // �����ε���������
    Eigen::Vector2f a(_v[0].x(), _v[0].y());
    Eigen::Vector2f b(_v[1].x(), _v[1].y());
    Eigen::Vector2f c(_v[2].x(), _v[2].y());

    // ���������ߵ�����
    Eigen::Vector2f ab = b - a;
    Eigen::Vector2f bc = c - b;
    Eigen::Vector2f ca = a - c;

    // ����㵽���������
    Eigen::Vector2f ap = p - a;
    Eigen::Vector2f bp = p - b;
    Eigen::Vector2f cp = p - c;

    // ʹ�ò���жϵ��Ƿ��������ߵ�ͬһ��
    float cross1 = ab.x() * ap.y() - ab.y() * ap.x();
    float cross2 = bc.x() * bp.y() - bc.y() * bp.x();
    float cross3 = ca.x() * cp.y() - ca.y() * cp.x();

    // ����������ͬ�ţ�������������ڲ�
    return (cross1 >= 0 && cross2 >= 0 && cross3 >= 0) ||
        (cross1 <= 0 && cross2 <= 0 && cross3 <= 0);
}

static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector3f* v)
{
    float c1 = (x*(v[1].y() - v[2].y()) + (v[2].x() - v[1].x())*y + v[1].x()*v[2].y() - v[2].x()*v[1].y()) / (v[0].x()*(v[1].y() - v[2].y()) + (v[2].x() - v[1].x())*v[0].y() + v[1].x()*v[2].y() - v[2].x()*v[1].y());
    float c2 = (x*(v[2].y() - v[0].y()) + (v[0].x() - v[2].x())*y + v[2].x()*v[0].y() - v[0].x()*v[2].y()) / (v[1].x()*(v[2].y() - v[0].y()) + (v[0].x() - v[2].x())*v[1].y() + v[2].x()*v[0].y() - v[0].x()*v[2].y());
    float c3 = (x*(v[0].y() - v[1].y()) + (v[1].x() - v[0].x())*y + v[0].x()*v[1].y() - v[1].x()*v[0].y()) / (v[2].x()*(v[0].y() - v[1].y()) + (v[1].x() - v[0].x())*v[2].y() + v[0].x()*v[1].y() - v[1].x()*v[0].y());
    return {c1,c2,c3};
}

void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    auto& buf = pos_buf[pos_buffer.pos_id];
    auto& ind = ind_buf[ind_buffer.ind_id];
    auto& col = col_buf[col_buffer.col_id];

    float f1 = (50 - 0.1) / 2.0;
    float f2 = (50 + 0.1) / 2.0;

    Eigen::Matrix4f mvp = projection * view * model;
    for (auto& i : ind)
    {
        Triangle t;
        Eigen::Vector4f v[] = {
                mvp * to_vec4(buf[i[0]], 1.0f),
                mvp * to_vec4(buf[i[1]], 1.0f),
                mvp * to_vec4(buf[i[2]], 1.0f)
        };
        //Homogeneous division
        for (auto& vec : v) {
            vec /= vec.w();
        }
        //Viewport transformation
        for (auto & vert : v)
        {
            vert.x() = 0.5*width*(vert.x()+1.0);
            vert.y() = 0.5*height*(vert.y()+1.0);
            vert.z() = vert.z() * f1 + f2;
        }

        for (int i = 0; i < 3; ++i)
        {
            t.setVertex(i, v[i].head<3>());
            t.setVertex(i, v[i].head<3>());
            t.setVertex(i, v[i].head<3>());
        }

        auto col_x = col[i[0]];
        auto col_y = col[i[1]];
        auto col_z = col[i[2]];

        t.setColor(0, col_x[0], col_x[1], col_x[2]);
        t.setColor(1, col_y[0], col_y[1], col_y[2]);
        t.setColor(2, col_z[0], col_z[1], col_z[2]);

        rasterize_triangle(t);
    }
}

// 将super-sampling样本解析到帧缓冲区
void rst::rasterizer::resolve_to_frame_buffer() {
    // 对每个像素，计算其sample_rate x sample_rate个样本的平均颜色
    for (int x = 0; x < width; ++x) {
        for (int y = 0; y < height; ++y) {
            int pixel_index = get_index(x, y);
            Eigen::Vector3f avg_color(0, 0, 0);
            int valid_samples = 0;
            
            // 计算所有样本的平均颜色
            for (int sx = 0; sx < sample_rate; ++sx) {
                for (int sy = 0; sy < sample_rate; ++sy) {
                    int sample_index = get_sample_index(x, y, sx, sy);
                    
                    // 只考虑深度测试通过的样本
                    if (sample_depth_buf[pixel_index][sample_index] < std::numeric_limits<float>::infinity()) {
                        avg_color += sample_color_buf[pixel_index][sample_index];
                        valid_samples++;
                    }
                }
            }
            
            // 如果有有效的样本，则计算平均颜色
            if (valid_samples > 0) {
                avg_color /= valid_samples;
                frame_buf[pixel_index] = avg_color;
            }
        }
    }
}

//Screen space rasterization - 支持2x2 super-sampling的实现
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    auto v = t.toVector4();

    // 计算三角形的bounding box
    float min_x = std::min({ v[0].x(), v[1].x(), v[2].x() });
    float max_x = std::max({ v[0].x(), v[1].x(), v[2].x() });
    float min_y = std::min({ v[0].y(), v[1].y(), v[2].y() });
    float max_y = std::max({ v[0].y(), v[1].y(), v[2].y() });

    // 限制bounding box在屏幕范围内
    min_x = std::max(0.0f, std::floor(min_x));
    max_x = std::min(static_cast<float>(width), std::ceil(max_x));
    min_y = std::max(0.0f, std::floor(min_y));
    max_y = std::min(static_cast<float>(height), std::ceil(max_y));

    // 遍历bounding box内的所有像素
    for (int x = static_cast<int>(min_x); x < max_x; x++) {
        for (int y = static_cast<int>(min_y); y < max_y; y++) {
            int pixel_index = get_index(x, y);
            
            // 对每个像素，进行2x2采样
            for (int sx = 0; sx < sample_rate; ++sx) {
                for (int sy = 0; sy < sample_rate; ++sy) {
                    // 计算样本点的坐标（像素中心偏移）
                    float sample_x = x + 0.5f * (sx + 0.5f) / sample_rate;
                    float sample_y = y + 0.5f * (sy + 0.5f) / sample_rate;
                    
                    // 检查样本点是否在三角形内
                    if (insideTriangle(sample_x, sample_y, t.v)) {
                        // 计算重心坐标并进行深度插值
                        auto [alpha, beta, gamma] = computeBarycentric2D(sample_x, sample_y, t.v);
                        float w_reciprocal = 1.0f / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                        float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                        z_interpolated *= w_reciprocal;
                        
                        // 获取样本索引
                        int sample_index = get_sample_index(x, y, sx, sy);
                        
                        // 深度测试
                        if (z_interpolated < sample_depth_buf[pixel_index][sample_index]) {
                            // 更新深度缓冲区
                            sample_depth_buf[pixel_index][sample_index] = z_interpolated;
                            
                            // 计算插值颜色
                            Eigen::Vector3f interpolated_color = t.getColor();
                            sample_color_buf[pixel_index][sample_index] = interpolated_color;
                        }
                    }
                }
            }
        }
    }
}

void rst::rasterizer::set_model(const Eigen::Matrix4f& m)
{
    model = m;
}

void rst::rasterizer::set_view(const Eigen::Matrix4f& v)
{
    view = v;
}

void rst::rasterizer::set_projection(const Eigen::Matrix4f& p)
{
    projection = p;
}

void rst::rasterizer::clear(rst::Buffers buff)
{
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        std::fill(frame_buf.begin(), frame_buf.end(), Eigen::Vector3f{0, 0, 0});
        
        // 清空super-sampling颜色缓冲区
        for (auto& pixel_samples : sample_color_buf) {
            std::fill(pixel_samples.begin(), pixel_samples.end(), Eigen::Vector3f(0, 0, 0));
        }
    }
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        std::fill(depth_buf.begin(), depth_buf.end(), std::numeric_limits<float>::infinity());
        
        // 清空super-sampling深度缓冲区
        for (auto& pixel_samples : sample_depth_buf) {
            std::fill(pixel_samples.begin(), pixel_samples.end(), std::numeric_limits<float>::infinity());
        }
    }
}

rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    frame_buf.resize(w * h);
    depth_buf.resize(w * h);
    
    // 初始化super-sampling缓冲区 - 每个像素包含sample_rate x sample_rate个样本
    sample_color_buf.resize(w * h);
    sample_depth_buf.resize(w * h);
    
    for (int i = 0; i < w * h; ++i) {
        sample_color_buf[i].resize(sample_rate * sample_rate, Eigen::Vector3f(0, 0, 0));
        sample_depth_buf[i].resize(sample_rate * sample_rate, std::numeric_limits<float>::infinity());
    }
}

// 获取像素在帧缓冲区中的索引
int rst::rasterizer::get_index(int x, int y)
{
    return (height-1-y)*width + x;
}

// 获取像素样本索引
int rst::rasterizer::get_sample_index(int x, int y, int sample_x, int sample_y)
{
    // 确保样本坐标在有效范围内
    if (sample_x < 0 || sample_x >= sample_rate || sample_y < 0 || sample_y >= sample_rate) {
        return -1; // 无效样本
    }
    
    // 计算样本在像素内的索引
    return sample_y * sample_rate + sample_x;
}

void rst::rasterizer::set_pixel(const Eigen::Vector3f& point, const Eigen::Vector3f& color)
{
    //old index: auto ind = point.y() + point.x() * width;
    auto ind = (height-1-point.y())*width + point.x();
    frame_buf[ind] = color;

}

// clang-format on